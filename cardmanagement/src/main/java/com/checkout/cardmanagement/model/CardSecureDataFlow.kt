    ...
    getSecureData(
        ...
        displaySecureData = {
            manager.service.displaySecurityCode(
                cardId = id,
                singleUseToken = singleUseToken,
                securityCodeViewConfiguration =
                    manager.designSystem.securityCodeViewConfig,
            )
        },
    )
...
/**
 * Copy PAN to clipboard with security features The PAN will only be copyable if it has first been
...
public fun Card.copyPan(
    ...
    if (currentAPIVersion >= Build.VERSION_CODES.Q && currentAPIVersion <= Build.VERSION_CODES.S_V2) {
        completionHandler(
            Result.failure(
                CardManagementError.UnsupportedAPIVersion(currentAPIVersion),
            ),
        )
        return
    }
...
/**
 * Result type that on success delivers a [AbstractComposeView] to be presented to the user, and on
 * failure delivers an error to identify problem
 */
...
/**
 * Result type that on success delivers a pair of [AbstractComposeView] for PAN & SecurityCode, and
