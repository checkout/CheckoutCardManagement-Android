...
internal fun Environment.parse() = when (this) {
...
internal fun CardState.getPossibleStateChanges(): List<CardState> = when (this) {
...
internal fun CardSuspendReason.toCardNetworkSuspendReason():
	com.checkout.cardnetwork.data.dto.CardSuspendReason =
	com.checkout.cardnetwork.data.dto.CardSuspendReason.values().first { it.name == this.name }

internal fun CardRevokeReason.toCardNetworkRevokeReason():
	com.checkout.cardnetwork.data.dto.CardRevokeReason =
	com.checkout.cardnetwork.data.dto.CardRevokeReason.values().first { it.name == this.name }
