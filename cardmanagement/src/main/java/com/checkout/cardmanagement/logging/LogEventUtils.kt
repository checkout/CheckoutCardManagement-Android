...
internal class LogEventUtils {
	...
	internal companion object {
		...
		private fun LogEvent.monitoringLevel(): MonitoringLevel = when (this) {
			...
			is LogEvent.PushProvisioning -> MonitoringLevel.INFO
		...
		private fun LogEvent.properties(
			...
			when (this) {
				...
				is LogEvent.GetPin -> {
					...
					propertyMap[KEY_CARD_STATE] = cardState.name.lowercase()
				}
...
private fun buildTextStyleMap(textStyle: TextStyle) = mutableMapOf<String, String>().apply {
	with(textStyle) {
		if (color != Color.Unspecified) put("color", color.toString())
		if (background != Color.Unspecified) put("background", background.toString())
		if (fontSize != TextUnit.Unspecified) put("fontSize", fontSize.toString())
		if (letterSpacing != TextUnit.Unspecified) put("letterSpacing", letterSpacing.toString())
		if (lineHeight != TextUnit.Unspecified) put("lineHeight", lineHeight.toString())
		if (fontWeight != null) put("fontWeight", fontWeight.toString())
		if (fontStyle != null) put("fontStyle", fontStyle.toString())
		if (fontSynthesis != null) put("fontSynthesis", fontSynthesis.toString())
		if (fontFamily != null) put("fontFamily", fontFamily.toString())
		if (fontFeatureSettings != null) put("fontFeatureSettings", fontFeatureSettings.toString())
		if (baselineShift != null) put("baselineShift", baselineShift.toString())
		if (textGeometricTransform != null) put("textGeometricTransform", textGeometricTransform.toString())
		if (localeList != null) put("localeList", localeList.toString())
		if (textDecoration != null) put("textDecoration", textDecoration.toString())
		if (shadow != null) put("shadow", shadow.toString())
		if (textAlign != null) put("textAlign", textAlign.toString())
		if (textDirection != null) put("textDirection", textDirection.toString())
		if (textIndent != null) put("textIndent", textIndent.toString())
	}
}
